		          	...
		          	<tbody>
						@foreach( $degreecourses as $degreecourse )
							<tr>
								...
								<td>{{ link_to_route('showDegreecourse_path', $degreecourse->name, $degreecourse->id) }}</td>
								...
								<td>
									{{ Form::open(array('class' => 'inline', 'method' => 'DELETE', 'route' => array('deleteDegreecourse_path', $degreecourse->id))) }}
										{{ HTML::decode(link_to_route('showDegreecourse_path', '<i class="glyphicon glyphicon-edit"></i>', array($degreecourse->id), array('class' => 'btn btn-xs btn-warning'))) }}
