@if (Entrust::hasRole('Admin') || Entrust::can('view_planning') || $currentUser->researchgroups->count() > 0)
	<li class="dropdown">
      	...
      	<ul class="dropdown-menu">
			<li>{{ link_to_route('showTurnPlannings_path', 'Planungen verwalten', $currentTurn) }}</li>

			@if (Entrust::hasRole('Admin') || $currentUser->researchgroups->count() > 0)
				<li>{{ link_to_route('showWeeklySchedule_path', 'Übersicht Wochenplan', $currentTurn) }}</li>
			@endif

			@if (Entrust::hasRole('Admin') || Entrust::can('view_room_preferences'))
				<li>{{ link_to_route('showRoomPreference_path', 'Übersicht Raumwünsche', $currentTurn) }}</li>
			@endif
		</ul>
	</li>
@endif